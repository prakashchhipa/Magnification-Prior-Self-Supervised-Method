...
def train_model(args_dict, fold):
    ...
    #ignore above settings
    model_weights_path = args_dict["pretrained_encoder"]["model_weights_path"][fold]
    ...
    if "imagenet" == pretraining_method_type:
        ...
        if "resnet" == encoder:
            ...
            if pretrained_weights_type == "pytorch_imagenet":
                ...
                downstream_task_model.model.load_state_dict(torch.load("/home/prachh/timm_models/resnet50_a1_0-14fe96d1.pth"), strict=False)
            elif pretrained_weights_type == "timm_swsl_imagenet":
                downstream_task_model.model.load_state_dict(torch.load("/home/prachh/timm_models/semi_weakly_supervised_resnet50-16a12f1b.pth"), strict=False)
            elif pretrained_weights_type == "timm_ssl_resnet":
                downstream_task_model.model.load_state_dict(torch.load("/home/prachh/timm_models/semi_supervised_resnet50-08389792.pth"), strict=False)    
                        ...
                        if _pretrained_batch_size in list(pretraining_batch_size_list):
                            for pretrained_epoch in list(os.listdir(os.path.join(pretraining_checkpoint_base_path, pretrained_encoder_dir, str(pretrained_batch_size)))):
                                ...
                                if _pretrained_epoch in list(pretraining_epochs_list):
                                    #Experiment description
                                    experiment_description = f"_{fold}_BISQUE_LE_{DP}_{encoder}{version}_{pretraining_method_type}_{pretraining_pair_sampling_method}_BS{pretrained_batch_size}_epoch{pretrained_epoch}_{pretraining_initial_weights}__LE{linear_eval}_"
                                    ...
                                    results = train_util.evaluate_testset_bach(model_weights_path)
                                    results_dict = {}
                                    results_dict["test"] = results
                                    filepath = os.path.join(result_stats_path, f"_eval_LE_{linear_eval}_{encoder}{version}_BISQUE_FT_{DP}_{pretraining_method_type}_{pretraining_pair_sampling_method}_{pretrained_batch_size}_{pretrained_epoch}.csv")
                                    file_exists = os.path.isfile(filepath)
                                    with open(filepath, 'a') as f:
                                        writer = csv.writer(f)
                                        if not file_exists:
